...
class Node(QtWidgets.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        painter.drawText(QtCore.QRect(self.x, self.y, 40, 40),
                         QtCore.Qt.AlignCenter, self.val)
...
class Edge(QtWidgets.QGraphicsItem):
    ...
    def get_directed_arrow_points(self, x1, y1, x2, y2, d):
        ...
        # given node radius d, we want to multiply the unit vector by d to get a
        ...
        # length from this point to edge is 1/2 radius of node
        point2 = (p1x+new_x*d/2.0, p1y+new_y*d/2.0)
        ...
        # length from this point to edge is 1/2 radius of node
        point3 = (p1x+new_x*d/2.0, p1y+new_y*d/2.0)
    ...
    def paint(self, painter, option, widget):
        ...
        if self.directed:  # if edge is part of a digraph
            ...
            point_array = self.get_directed_arrow_points(
                self.x1, self.y1, self.x2, self.y2, 20)  # get coordinates of arrow vertices
            ...
            arrow = QtGui.QPolygonF(points)
            painter.drawPolygon(arrow)  # draw arrow

            # if digraph, draw edge weight closer to arrow
            # get x coordinate of point between distination node and edge midpoint
            quartX = (self.midx+self.x2)/2
            quartY = (self.midy+self.y2)/2  # get y component for same point
            # string to indicate towards which node this weight applies to
            strDirection = ':>'+str(self.node2.val)
            painter.setPen(QtCore.Qt.black)  # set pen color to black
            painter.setFont(QtGui.QFont('Decorative', 9))  # set font
            painter.drawText(quartX, quartY, self.strWeight +
                             strDirection)  # draw weight

        else:
            painter.setPen(QtCore.Qt.black)  # pen to black
            painter.setFont(QtGui.QFont('Decorative', 11))  # set font
            # draw weight near mipoint of edge
            painter.drawText(self.midx - 5*(len(self.strWeight)),
                             self.midy, self.strWeight)
...
class GraphScene(QtWidgets.QGraphicsScene):
    def __init__(self, digraph):
        ...
        # graph object to underlay the graphical interface
        self.graph = graph.Graph(self.digraph)

        # initialize information about displayed path
        self.path_displayed = (False, 'NONE', 'NONE', 'NO PATH')
        # set current path algorithm being used to DIJKSTRA
        self.current_path_algo = 'DIJKSTRA'

        # create a data updater to send out a signal anytime data about the graph is changed
    ...
    def delete_edge_selected(self):
        ...
        if self.check_selected(2):  # if nodes selected
            ...
            self.remove_edge(self.selected[0].val, self.selected[1].val)
    ...
    def add_edge_selected(self):
        ...
        if self.check_selected(2):  # if nodes selected
            ...
            if ok:
                ...
                if self.add_edge(self.selected[0].val, self.selected[1].val, weight):
                    self.deselect_nodes()  # deselect nodes
    ...
    def display_path(self):
        if self.current_path_algo == 'PRIMS':  # if current path algorithm is prims
            ...
            # if nodes are selected run dijktra between them
            if self.check_selected(2):
                self.show_shortest_path_dijkstra(
                    self.selected[0].val, self.selected[1].val)
                self.deselect_nodes()  # deselect nodes
            else:  # else print error
                self.InvalidInMsg.setText(
                    'Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
        elif self.current_path_algo == 'BELLMAN FORD':  # if current algorithm is bellman ford
            # if nodes are selected run bellman ford between them
            if self.check_selected(2):
                self.show_shortest_path_bellman_ford(
                    self.selected[0].val, self.selected[1].val)
                self.deselect_nodes()  # deselect nodes
            else:  # else print error
                self.InvalidInMsg.setText(
    ...
    def select_node(self, event):
        ...
        node = self.itemAt(event.scenePos(), QtGui.QTransform())
    ...
    def add_node(self, event):
        ...
        if ok:  # dialog value was input
            # if input was between 1 and 4 characters
            if len(str(node_val)) < 5 and len(str(node_val)) > 0:
                connections = []
                # get wether or not a shortest path is currently displayed
                ...
                if node_val in self.nodes:  # node being added already exists
                    # remove original node and save all its node connections
                    connections = self.remove_node(node_val)

                # create a new node at the given x and y coordinates
                node = Node(x-20, y-20, str(node_val))
                self.addItem(node)  # add node to scene
                self.nodes[node.val] = node  # add node to node dictionary
                # add node value to underlying graph objects
                ...
                # print message if invalid dialog input
                self.InvalidInMsg.setText(
                    'Node name must consist of between 1 and 4 characters')
                self.InvalidInMsg.exec_()
                return

            # reset path displayed variable to reflect its original value before node was added
            self.path_displayed = (
                path_shown, self.path_displayed[1], self.path_displayed[2], self.path_displayed[3])
            if self.path_displayed[0]:  # if path was displayed before node was added
                self.reset_path()  # find and display path

            # emit a signal to notify that the graph was updated
    ...
    def add_edge(self, node1_val, node2_val, weight):
        ...
        if node2_val == node1_val:  # ensure node values are unique
            self.InvalidInMsg.setText(
                'Two unique node values required to create an edge')
            self.InvalidInMsg.exec_()
            return False
        ...
        # save whether shortest path is being shown
        path_shown = self.path_displayed[0]
        self.delete_shortest_path()  # delete shortest path
        # if edge already exists between given nodes
        if (node1_val, node2_val) in self.edges or ((node2_val, node1_val) in self.edges and not self.digraph):
        ...
        # add edge to underlying graph
        self.graph.add_edge(node1.val, node2.val, numWeight)
        ...
        self.edges[(node1_val, node2_val)] = edge

        # reset path displayed value
        self.path_displayed = (
            path_shown, self.path_displayed[1], self.path_displayed[2], self.path_displayed[3])
    ...
    def remove_edge(self, node1_val, node2_val):
        ...
        # if edge from node1_val to node2_val not in edges dictionary
        if (node1_val, node2_val) not in self.edges:
            # and edge from node2_val to node1_val not in edges dictionary
            if self.digraph or (node2_val, node1_val) not in self.edges:
                self.InvalidInMsg.setText(
                    'No edge exists between nodes ' + str(node1_val) + ' and ' + str(node2_val))
                self.InvalidInMsg.exec_()  # print message and exit
                return
            else:
                # otherwise represent edge from node2_val, node1_val
                edge = self.edges[(node2_val, node1_val)]
        else:
            # otherwise represent edge from node1_val, node2_val
        ...
        # remove edge from underlaying graph
        self.graph.remove_edge(node1_val, node2_val)

        # delete edge from edges dictionary
        del self.edges[(edge.node1.val, edge.node2.val)]
    ...
    def remove_node(self, node_val):
        ...
        for node_pair in self.edges.keys():  # for each edge in graph
            # if edge connects to this node
            if node_pair[0] == node_val or node_pair[1] == node_val:
                # save the connection in list
                connections.append(
                    (node_pair[0], node_pair[1], self.edges[node_pair].weight))

        for connection in connections:  # for all connections
            # remove edges from graph
            self.remove_edge(connection[0], connection[1])

        self.removeItem(self.nodes[node_val])  # remove the node from the scene
        # remove the node from the underlaying graph
        self.graph.remove_node(node_val)
    ...
    def show_shortest_path_dijkstra(self, from_node_val, to_node_val):
        ...
        short_path_info = path_alg.dijkstra(self.graph, from_node_val, [to_node_val])[
            0]  # call dijkstra algorithm on underlaying graph

        if short_path_info[1] < 0:
            self.InvalidInMsg.setText(
                'DIJKSTRA requires connected edges to be positive')
            self.InvalidInMsg.exec_()  # show message and exit
            return
        # get the path list from running that path search
        ...
        while len(path) > 1:  # while length of path if greater than 1
            ...
            if len(path) > 0:  # if length of path is still greater than 0
                # and edge exists between current node value and next in path
                if (node_val, path[0]) in self.edges:
                    # highlight the edge
                    self.edges[(node_val, path[0])].highlighted = True
                else:
                    # else the edge exists as being from next in path to current node
                    self.edges[(path[0], node_val)].highlighted = True
        # highlight the last node in the path
        self.nodes[path[0]].highlighted = True

        if self.digraph:
            self.overlay_highlighted()
        self.update()
        self.path_displayed = (True, from_node_val, to_node_val, str(
            short_path_info[1]))  # reset path displayed information
    ...
    def show_shortest_path_bellman_ford(self, from_node_val, to_node_val):
        ...
        # if nodes for path not in nodes dictionary
        if from_node_val not in self.nodes or to_node_val not in self.nodes:
            self.InvalidInMsg.setText('Invalid node value input')
            self.InvalidInMsg.exec_()  # show message and exit
            return

        short_path_info = path_alg.bellman_ford(self.graph, from_node_val, [to_node_val])[
            0]  # call bellman ford on underlaying graph
    ...
    def show_mst_prims(self):
        if self.digraph:
            self.InvalidInMsg.setText(
                'Cannot perform Prims algorithm on a directed graph')
            self.InvalidInMsg.exec_()  # show message
            return

        self.delete_shortest_path()  # delete shortest path of currently displayed

        # call dijkstra algorithm on underlaying graph
        mst_edges = path_alg.prims(self.graph)

        if mst_edges == None:
            self.InvalidInMsg.setText(
                'Graph must be connected to perform Prims algorithm')
            self.InvalidInMsg.exec_()  # show message
            return
        dist = 0
        ...
        for edge in mst_edges:
            # and edge exists between current node value and next in path
            if (edge[1], edge[2]) in self.edges:
                # highlight the edge
                self.edges[(edge[1], edge[2])].highlighted = True
            else:
                # else the edge exists as being from next in path to current node
                self.edges[(edge[2], edge[1])].highlighted = True
            dist = dist + edge[0]

        if self.digraph:
            self.overlay_highlighted()
        self.update()
        # reset path displayed information
        self.path_displayed = (True, 'N/A', 'N/A', str(dist))
    ...
    def delete_shortest_path(self):
        ...
        # path info not shown
        self.path_displayed = (
            False, self.path_displayed[1], self.path_displayed[2], self.path_displayed[3])
    ...
    def reset_path(self):
        ...
        if self.current_path_algo == 'PRIMS':  # if current algorithm is prims
            ...
            self.show_shortest_path_dijkstra(
                self.path_displayed[1], self.path_displayed[2])  # run dijskstra
        elif self.current_path_algo == 'BELLMAN FORD':  # if current algorithm is bellman_ford
            self.show_shortest_path_bellman_ford(
                self.path_displayed[1], self.path_displayed[2])  # run bellman ford
    ...
    def overlay_highlighted(self):
        for (from_node_val, to_node_val), edge in self.edges.items():
            if edge.highlighted:
                ...
                # layer highlighted edges over none highlighted edges
                self.addItem(edge)
