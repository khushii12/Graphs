...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        sizePolicy.setHeightForWidth(
            self.path_status_header.sizePolicy().hasHeightForWidth())
        ...
        self.path_status_header.setAlignment(
        ...
        self.path_algorithm_header_lab.setObjectName(
            "path_algorithm_header_lab")
        ...
        self.select_path_alg_comboBox = SceneConnectedComboBox(
            self.centralwidget, self.scene)  # box requires reference to scene
        self.select_path_alg_comboBox.setEditable(False)
        self.select_path_alg_comboBox.addItems(
            ['DIJKSTRA', 'BELLMAN FORD', 'PRIMS'])  # algorithms that can be run
        ...
        # add combo box to vertical layout
        self.verticalLayout.addWidget(self.select_path_alg_comboBox)
        ...
        self.graph_info_lab.setAlignment(
            QtCore.Qt.AlignBottom | QtCore.Qt.AlignHCenter)
        ...
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(
            self.graph_status.sizePolicy().hasHeightForWidth())
        ...
        # action pulling up a help menu
        helpAction = QtWidgets.QAction('&Open Help', self.MainWindow)
        helpAction.setShortcut('Ctrl+H')
        helpAction.setStatusTip('application help')
        helpAction.triggered.connect(lambda: webbrowser.open(
            "GraphGuiHelp.pdf"))  # open help file
    ...
    def button_setup(self):
        ...
        self.select_path_alg_comboBox.activated.connect(
            self.edit_path_algorithm)

        # connect the MainWindows init_control_panel function to button
        self.control_panel_btn.clicked.connect(
            lambda: self.MainWindow.init_control_pane())
    ...
    def change_graph_type(self):
        ...
        # switch graph type and get previously used nodes and edges from old graph
        (nodes, edges) = self.MainWindow.switch_graph_type()
        # reset the UI with the modified MainWindow
        self.setupUi(self.MainWindow)
        if self.scene.digraph:  # if changed to digraph
            self.switch_graph_btn.setText(_translate(
                "MainWindow", "TO GRAPH"))  # button to change to graph
        else:
            # button to change to digraph
            self.switch_graph_btn.setText(
                _translate("MainWindow", "TO DIGRAPH"))
        ...
        for node_val, node in nodes.items():  # for each node that was in the original graph
            ...
            # add the node to the scene's dictionary
            self.scene.nodes[node_val] = node
            # add the node to the scene's underlying graph
            self.scene.graph.add_node(node_val)

        for (from_node, to_node), edge in edges.items():  # for each edge in the original graph
            # add it to the new graph
            self.scene.add_edge(from_node, to_node, edge.weight)
            if self.scene.digraph:  # if changed from graph to digraph
                # add edges in both directions
                self.scene.add_edge(to_node, from_node, edge.weight)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.path_status_header.setText(
            _translate("MainWindow", "-PATH STATUS-"))
        self.path_algorithm_header_lab.setText(
            _translate("MainWindow", "Path Algorithm:"))
        ...
        self.control_panel_btn.setText(
    ...
    def update_data(self):
        ...
        # if a path is being displayed indicate that it is and fill in relavent data from graph
        if self.scene.path_displayed[0]:
            self.path_shown_yes_no.setText(_translate("MainWindow", "YES"))
            self.node1_val_lab.setText(_translate(
                "MainWindow", str(self.scene.path_displayed[1])))
            self.node2__val_lab.setText(_translate(
                "MainWindow", str(self.scene.path_displayed[2])))
            self.dist_val_lab.setText(_translate(
                "MainWindow", str(self.scene.path_displayed[3])))
        ...
        self.num_nodes_val.setText(_translate(
            "MainWindow", str(len(self.scene.nodes))))
        self.num_edges_val.setText(_translate(
            "MainWindow", str(len(self.scene.edges))))
...
class SceneConnectedComboBox(QtWidgets.QComboBox):
    ...
    def keyPressEvent(self, event):
        # if the combobox is selected and a key is pressed send event to graph scene
...
class MainGraphWindow(QtWidgets.QMainWindow):
    ...
    def init_control_pane(self):
        # main window take up 3/4 of the total width of the screen
        self.setGeometry(self.width/4+5, 40, 3*self.width/4, self.height-100)
        # create an new control panel window
        self.GraphControlWindow = QtWidgets.QWidget()
        ui = GraphControlPanel()
        ui.setupUi(self.GraphControlWindow, self.graph_scene)
        # control panel takes 1/4 of the total width of the screen
        self.GraphControlWindow.setGeometry(
            0, 40, self.width/4, self.height-100)
    ...
    def closeEvent(self, event):
        # when the main window is closed the control panel must also close
        self.GraphControlWindow.close()
