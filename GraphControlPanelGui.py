...
class Ui_GraphControlWindow(object):
    def setupUi(self, GraphControlWindow, graph_scene):
        ...
        self.add_edge_lab.setAlignment(
        ...
        self.remove_edge_lab.setAlignment(
            QtCore.Qt.AlignBottom | QtCore.Qt.AlignHCenter)
        ...
        self.remove_node_lab_2.setAlignment(
            QtCore.Qt.AlignBottom | QtCore.Qt.AlignHCenter)
        ...
        self.display_path_header.setAlignment(
            QtCore.Qt.AlignBottom | QtCore.Qt.AlignHCenter)
    ...
    def init_buttons(self):
        ...
        self.remove_node_btn.clicked.connect(lambda: self.scene.remove_node(
            self.remove_node_edit.text()))  # remove node

        # show path using selected algorithm
        self.find_path_btn.clicked.connect(lambda: self.show_selected_path())

        self.clear_path_btn.clicked.connect(
            lambda: self.scene.delete_shortest_path())  # clear path

    def show_selected_path(self):
        # get the current algorithm being used by GraphScene
        algo = self.scene.current_path_algo

        if algo == 'DIJKSTRA':  # if using dijkstra
            # run dijkstra on GraphScene with values from text edit boxes
            self.scene.show_shortest_path_dijkstra(
                self.path_node1_edit.text(), self.path_node2_edit.text())
            ...
            self.scene.show_shortest_path_bellman_ford(
                self.path_node1_edit.text(), self.path_node2_edit.text())
    ...
    def retranslateUi(self, GraphControlWindow):
        ...
        GraphControlWindow.setWindowTitle(_translate(
            "GraphControlWindow", "Graph Control WIndow"))
        self.add_edge_lab.setText(_translate(
            "GraphControlWindow", "-Add Edge-"))
        ...
        self.add_edge_weight_lab.setText(
            _translate("GraphControlWindow", "Weight:"))
        self.add_edge_btn.setText(_translate("GraphControlWindow", "ADD EDGE"))
        self.remove_edge_lab.setText(_translate(
            "GraphControlWindow", "-Delete Edge-"))
        self.remove_edge1_lab.setText(
            _translate("GraphControlWindow", "Node 1:"))
        self.remove_edge2_lab.setText(
            _translate("GraphControlWindow", "Node 2:"))
        self.remove_edge_btn.setText(_translate(
            "GraphControlWindow", "REMOVE EDGE"))
        self.remove_node_lab_2.setText(_translate(
            "GraphControlWindow", "-Delete Node-"))
        self.remove_node_lab.setText(_translate("GraphControlWindow", "Node:"))
        self.remove_node_btn.setText(_translate(
            "GraphControlWindow", "REMOVE NODE"))
        self.display_path_header.setText(_translate(
            "GraphControlWindow", "-Display Path-"))
        self.path_node1_lab.setText(
            _translate("GraphControlWindow", "Node 1:"))
        self.path_node2_lab.setText(
            _translate("GraphControlWindow", "Node 2:"))
        self.find_path_btn.setText(_translate(
            "GraphControlWindow", "SHOW PATH"))
        self.clear_path_btn.setText(_translate(
            "GraphControlWindow", "CLEAR PATH"))
